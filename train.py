...
opt = argparse.Namespace(attn_mode='before', batch_size=8, epochs=50, log_images=True, lr=0.1, no_attention=False,
                         normalize_attn=True, outf=d_logs)
print(opt)

#d_train = os.path.join(d_logs, 'train')
#d_test = os.path.join(d_logs, 'test')

for d in [d_logs]: #, d_train, d_test]:
        ...
        for aug in range(num_aug):
            for i, data in enumerate(trainloader, 0):
                ...
                if i % 10 == 0:
                    ...
                    # print("accuracy:%s = correct:%s [predict:%s, labels:%s] / total:%s" % (accuracy, correct, predict, labels, total))
